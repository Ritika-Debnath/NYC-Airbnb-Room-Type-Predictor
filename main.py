...
def predict(features: Features):
    row = pd.DataFrame([features.model_dump()], columns=COLUMNS)   #STEP1:- here we converts features into dictionary, after getting all correct features from users it stors in "row"
    prediction  = model.predict(row)          #Then the "model" will predict from "row"
    probability = model.predict_proba(row)     #"model" will give the probabilty of "room_type"
